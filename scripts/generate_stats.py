...
Env:
  ...
  GH_LOGIN      user to summarise (default: 7-Bala)
...
# The hero counts (total contributions, streak lengths) get a rounder,
# friendlier numeral face — same family as the compass wordmark's "explore" —
# so the one number on the page meant to catch your eye doesn't read as
# generic dashboard-bold JetBrains Mono.
NUM = "FredokaNum,ui-rounded,system-ui,sans-serif"
FONT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fonts")


@functools.lru_cache(maxsize=None)
def face(filename, weight, family="JBMono"):
...
def font_text_num():
    """font_text(), plus the rounded digit face for a graphic's hero number."""
    return font_text() + face("fredoka-digits.woff2", 600, "FredokaNum")
...
def draw_stats(s):
    ...
    p = [head(WIDTH, H, font=font_text_num())]
    p.append(f'<g opacity="0">{fade(0.10)}'
             + label(0, 50, s["total"], 52, "e-f",
                     extra=f' font-weight="600" font-family="{NUM}"')
    ...
    for i, (val, lab) in enumerate([(s["active"], "active days"),
        ...
        p.append(f'<g opacity="0">{fade(0.30 + i * 0.12)}'
                 + label(WIDTH, 30 + i * 40, val, 19, "e-f", "end",
                         f' font-weight="600" font-family="{NUM}"')
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "7-Bala")
